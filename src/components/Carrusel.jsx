      ...
      <section className="py-12 sm:py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8 sm:mb-12">
            ...
            <div className="h-5 sm:h-6 w-56 sm:w-96 bg-gray-200 rounded-lg animate-pulse mx-auto"></div>
          </div>
          <div className="relative overflow-hidden py-4">
            <div className="flex justify-center gap-3 sm:gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 bg-gray-200 rounded-xl animate-pulse"></div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          >
            {marcas.map((marca, index) => (
              <SwiperSlide key={`marca-${index}`}>
                <div className="flex items-center justify-center p-2 sm:p-3 md:p-4">
                  <img
                    src={marca.logo || '/images/teknisolution.webp'}
                    alt={marca.nombre || `Marca ${index + 1}`}
                    className="w-auto h-12 sm:h-14 md:h-16 lg:h-20 max-w-full object-contain transition-all duration-300 hover:scale-110 cursor-pointer"
                    title={marca.nombre}
                    loading="lazy"
                    decoding="async"
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = '/images/teknisolution.webp';
                    }}
                  />
                </div>
              </SwiperSlide>
