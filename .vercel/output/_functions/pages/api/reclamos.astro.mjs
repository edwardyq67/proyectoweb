...
function obtenerTipoDocumentoNombre(tipo) {
  const tipos = {
    "dni": "DNI",
    "ruc": "RUC",
...
function obtenerTipoReclamoNombre(tipo) {
  const tipos = {
    "reclamo": "Reclamo",
    "queja": "Queja"
  };
...
const POST = async ({ request }) => {
  try {
    ...
    const bienServicio = formData.get("bienServicio");
    const montoReclamado = formData.get("montoReclamado");
    const descripcionBien = formData.get("descripcionBien");
    const tipoReclamo = formData.get("tipoReclamo");
    const fechaHecho = formData.get("fechaHecho");
    ...
    const fechaRegistro = (/* @__PURE__ */ new Date()).toLocaleString("es-PE");
    const fechaFormato = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    ...
    const emailEmpresa = await resend.emails.send({
      ...
      to: [empresa.email, "servicios@soflendil.pe"],
